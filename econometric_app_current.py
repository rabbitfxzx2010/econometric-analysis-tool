...
def generate_python_code(model, estimation_method, independent_vars, dependent_var, 
                        ...
                        random_state=42, output_format='notebook'):
    """
    ...
    if use_stratify and model_type == 'classification':
        code_lines.extend([
            ...
            "    X, y, test_size=0.2, random_state={}, stratify=y".format(random_state),
            ")"
        ])
    else:
        code_lines.extend([
            "X_train, X_test, y_train, y_test = train_test_split(",
            f"    X, y, test_size=0.2, random_state={random_state}",
            ")"
        ])
...
This notebook replicates your exact analysis from the econometric app, including all preprocessing steps, model configuration, and evaluation metrics."""
    ...
    checklist_content = create_options_checklist(
        ...
        use_sample_filter, random_state
    )
    ...
    # Missing data handling cell (if applicable)
    if missing_data_method and missing_data_method != "Listwise Deletion":
        missing_content = create_missing_data_section(missing_data_method)
        
        notebook["cells"].append({
            "cell_type": "markdown",
            "metadata": {},
            "source": ["## 🔧 Missing Data Handling", f"", f"Method: **{missing_data_method}**"]
        })
    ...
    model_content = create_model_training_section(
        ...
        class_weight, use_stratify, random_state
    )
...
- {'✅' if missing_data_method else '❌'} **Missing Data:** {missing_data_method if missing_data_method else 'None'}
            ...
            if independent_vars:
                ...
                if st.sidebar.button(f"🔬 Run {estimation_method} Regression", type="primary"):
                    ...
                    if estimation_method in ["Decision Tree", "Random Forest"]:
                        ...
                        # Code Generation Section for Tree Models
                        ...
                        generated_code = generate_python_code(
                            ...
                            standardize_data=standardize_data if 'standardize_data' in locals() else False,
                            ...
                            min_samples_split=min_samples_split if 'min_samples_split' in locals() else 2,
                            min_samples_leaf=min_samples_leaf if 'min_samples_leaf' in locals() else 1,
                            enable_pruning=enable_pruning if 'enable_pruning' in locals() else False,
                            pruning_method=pruning_method if 'pruning_method' in locals() else None,
                            manual_alpha=manual_alpha if 'manual_alpha' in locals() else None,
                            use_max_depth=use_max_depth if 'use_max_depth' in locals() else True,
                            prob_class_index=prob_class_index if 'prob_class_index' in locals() else 0,
                            include_plots=True,
                            parameter_input_method=None,  # Not used for tree models
                            use_stratify=use_stratify if 'use_stratify' in locals() else False,
                            class_weight_option=class_weight_option if 'class_weight_option' in locals() else None,
                            filter_method=filter_method if 'filter_method' in locals() else None,
                            start_row=start_row if 'start_row' in locals() else None,
                                end_row=end_row if 'end_row' in locals() else None,
                                use_sample_filter=use_sample_filter if 'use_sample_filter' in locals() else False,
                            random_state=42,
                            output_format=output_format
                        )
                        
                        # Display notebook content in main window
                        st.json(json.loads(generated_code))
                        ...
                        st.download_button(
                            ...
                            key="download_tree_notebook"
                                ...
                                st.caption("� **Python Script**: Copy this code to reproduce your analysis in any Python environment.")
                                
                                # Download button for Python code
                                st.download_button(
                                    label="📥 Download Python Script",
                                    data=generated_code,
                                    file_name=f"{estimation_method.lower().replace(' ', '_')}_analysis.py",
                                    mime="text/plain",
                                    key="download_tree_code"
                                )
                        ...
                        **Model Equation ({estimation_method}):**
                        ...
                        generated_code = generate_python_code(
                                ...
                                include_plots=True,
                                parameter_input_method=parameter_input_method if 'parameter_input_method' in locals() else None,
                                use_stratify=use_stratify if 'use_stratify' in locals() else False,
                                class_weight_option=class_weight_option if 'class_weight_option' in locals() else None,
                                filter_method=filter_method if 'filter_method' in locals() else None,
                                start_row=start_row if 'start_row' in locals() else None,
